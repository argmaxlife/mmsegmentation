...
def main():
    ...
    colors = [(128, 128, 128), (0, 255, 255), (255, 0, 0)]
    ...
    for file in tqdm(os.listdir(input_dir)):
        ...
        if ann is not None:
            ...
            if gt_area > 0:
                gt_pred_area = np.count_nonzero(gt_mask & pred_mask)
                hit_rate = gt_pred_area / gt_area
                if hit_rate > 0.3:
                    tp += 1
                    row = [file, 1, 1, 1]
                    ws.append(row)
            ...
            gt_bj_area = np.count_nonzero(gt_bjbmyw_mask)
            if gt_bj_area > 0:
                gt_pred_bj_area = np.count_nonzero(gt_bjbmyw_mask & pred_bjbmyw_mask)
                bj_hit_rate = gt_pred_bj_area / gt_bj_area
                if bj_hit_rate > 0:
                    tp_bj += 1
                else:
                    fn_bj += 1
            ...
            gt_dm_area = np.count_nonzero(gt_dmyw_mask)
            if gt_dm_area > 0:
                gt_pred_dm_area = np.count_nonzero(gt_dmyw_mask & pred_dmyw_mask)
                dm_hit_rate = gt_pred_dm_area / gt_dm_area
                if dm_hit_rate > 0:
                    tp_dm += 1
                else:
                    fn_dm += 1
    ...
    print(f"precision_bj = {precision_bj}, recall_bj = {recall_bj}")
    print(f"precison_dm = {precison_dm}, recall_dm = {recall_dm}")
    ...
    xlsx = checkpoint.parent / f"{input_dir.stem}.xlsx"
    wb.save(xlsx)
